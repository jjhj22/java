<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <c:choose>
    	...
    	<c:otherwise>
    		나는 위에 모든 when이 모두 거짓일 경우에 실행된다
    	</c:otherwise>
    </c:choose>
	...
	<%--
		웹 모델1 : 사용자 요청 -> 서버 -> 컨테이너 -> 서블릿 -> 페이지 처리
			     -> 컨테이너 -> 서버 -> 사용자
		...
		예) 회원가입을 위해 아이디,비번,이름,주소,연락처 등을 입력하고
			가입 버튼을 클릭
			...
			5. Controller는 가입 완료 페이지를 사용자에게 보낸다
			
			사용자가 로그인을 한다. 아이디 비번 입력하고 로그인 버튼 클릭
			
			1. 로그인 요청을 Controller가 받는다
			2. 로그인 처리를 해줄 수 있는 service를 실행한다
			3. service에서 데이터베이스 작업을 수행하는 DAO실행
			4. DAO실행 결과 아이디 비번 일치하면 service에서 세션생성
